...
public class ReviewDislikeDbStorage extends BaseStorage<ReviewLike> implements ReviewDislikeStorage {
    private static final String GET_REVIEW_DISLIKE_QUERY = "SELECT * from reviews_dislikes WHERE review_id = ? AND user_id = ?";
    ...
    public Optional<ReviewLike> getDislike(int reviewId, int userId) {
        return findOne(GET_REVIEW_DISLIKE_QUERY, reviewId, userId);
    }
